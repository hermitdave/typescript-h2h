...
export function taskCmp(a: TaskHeapEntry, b: TaskHeapEntry): number {
...
export class TaskHeap {
  private readonly h: TaskHeapEntry[] = [];
  ...
  push(id: string, due: number, prio: number, seq: number): void {
    if (this.pos.has(id)) throw new Error(`TaskHeap.push: duplicate id ${id}`);
  ...
  key(id: string): { due: number; prio: number } | undefined {
