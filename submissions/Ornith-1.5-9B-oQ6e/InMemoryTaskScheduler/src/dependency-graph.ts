...
export class DependencyGraph {
  ...
  private readonly dependentsByTask = new Map<string, Set<string>>();
  /** Task -> its set of hard dependencies. */
  private readonly dependenciesByTask = new Map<string, Set<string>>();
  ...
  registerTask(taskId: string): void {
    ...
    if (!this.dependentsByTask.has(taskId)) this.dependentsByTask.set(taskId, new Set());
  }
  ...
  addDependency(taskId: string, dep: string): void {
    ...
    if (newEdges) {
      ...
      if (cycle) throw new DependencyResolutionError(taskId, cycle);
  ...
  removeDependency(taskId: string, dep: string): void {
    if (this.dependenciesByTask.get(taskId)?.delete(dep)) this.unwire(taskId, dep);
  }
  ...
  replaceDependencies(taskId: string, deps: readonly string[]): void {
    ...
    for (const dep of this.dependenciesByTask.get(taskId) ?? []) this.unwire(taskId, dep);
    for (const dep of deps) this.addDependency(taskId, dep);
  }
  ...
  wouldCreateCycle(taskId: string, dep: string): string[] | null {
    ...
    if (this.dependenciesByTask.get(taskId)?.has(dep)) return null;
    const path: string[] = [taskId];
    const visited = new Set<string>([taskId]);
    const stack = [taskId];
    while (stack.length > 0) {
      const cur = stack.pop()!;
      if (cur === dep) return path; // path from taskId ... dep
  ...
  findCyclicTasks(): string[] {
    ...
    const queue: string[] = [];
    for (const [task, deg] of remaining) if (deg === 0) queue.push(task);
  ...
  // ----- internals -----
  private require(taskId: string, dep: string): void {
    if (!this.registeredTasks.has(taskId)) throw new InvalidConfigurationError(`Unknown task in dependency graph: "${taskId}"`);
    if (!this.registeredTasks.has(dep)) throw new InvalidConfigurationError(`Dependency on unknown/unregistered task: "${dep}"`);
  }

  private requireExisting(taskId: string): void {
    if (!this.registeredTasks.has(taskId)) throw new InvalidConfigurationError(`Unknown task in dependency graph: "${taskId}"`);
  }

  /** `taskId` depends on `dep`: add both edges. */
  private wire(taskId: string, dep: string): void {
  ...
  private unwire(taskId: string, dep: string): void {
