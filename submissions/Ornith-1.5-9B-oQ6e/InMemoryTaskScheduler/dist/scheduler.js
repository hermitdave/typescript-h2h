...
import { SchedulerEvent, TaskStatus, TERMINAL_STATUSES, TASK_FAILED_STATUSES, } from './types';
/**
...
export class Scheduler {
    ...
    runOnce() {
        ...
        task.startedAt = this.clock();
        task.executions += 1;
        return Promise.resolve(this._executeAttempt(task)).then((out) => {
            this.runningTaskId = null;
            return out;
        }, (err) => {
            this.runningTaskId = null;
     ...
     */
    _executeAttempt(task) {
        const resultPromise = Promise.resolve(task.handler(task.args));
    ...
    reconcile(id) {
        ...
        if (TERMINAL_STATUSES.has(task.status))
            return; // already terminal/faulted
    ...
    _readiness(id) {
        ...
        for (const depId of task.deps) {
            ...
            if (depTask.status === TaskStatus.Success)
                continue;
    ...
    _stuck(id, blockedBy) {
        ...
        if (!task)
            return;
        if (task.status === TaskStatus.Stuck || TERMINAL_STATUSES.has(task.status))
            return;
    ...
    _skip(id, blockedBy) {
        ...
        if (!task)
            return;
        if (task.status === TaskStatus.Skipped || TERMINAL_STATUSES.has(task.status))
            return;
    ...
    _stuckBlockedBy(id) {
        ...
        for (const depId of task.deps) {
            const depTask = this.tasks.get(depId);
            if (depTask && depTask.status !== TaskStatus.Success)
                return depId;
        }
        return null;
    }
    ...
    updateDeps(id, newDeps) {
        ...
        if (task.status === TaskStatus.Running || TERMINAL_STATUSES.has(task.status))
            return;
        this.reconcile(id);
    }
    ...
    updateMaxAttempts(id, maxAttempts) {
        const task = this.getTask(id);
