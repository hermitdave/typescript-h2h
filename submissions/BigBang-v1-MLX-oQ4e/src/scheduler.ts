...
function readyComparator(a: HeapEntry, b: HeapEntry): number {
...
function deferredComparator(a: HeapEntry, b: HeapEntry): number {
...
export class TaskScheduler {
  ...
  private forward = new Map<string, Set<string>>();   // task -> its dependencies
  private reverse = new Map<string, Set<string>>();   // task -> dependents

  private readyHeap = new BinaryHeap<HeapEntry>(readyComparator);
  private deferredHeap = new BinaryHeap<HeapEntry>(deferredComparator);
  ...
  // ------------------------------------------------------------------ time
  setTime(now: number): void { this.now = now; }
  ...
  addTask(input: TaskInput): string {
    ...
    const deps = new Set<string>(input.deps ?? []);
    if (deps.has(id)) throw new Error(`Task ${id} cannot depend on itself`);
    if (this.detectCycle(id, deps)) throw new Error(`Cycle detected for task ${id}`);
    ...
    const task: Task = {
      ...
      createdAt: Date.now(),
      version: 0,
      depStatus: new Map<string, DepStatus>(),
    };
    ...
    for (const dep of deps) {
      const rev = this.reverse.get(dep) ?? new Set<string>();
      rev.add(id);
  ...
  updateTask(id: string, patch: TaskPatch): void {
    ...
    if (newDeps.has(id)) throw new Error(`Task ${id} cannot depend on itself`);
    if (this.detectCycle(id, newDeps)) throw new Error(`Cycle detected for task ${id}`);
  ...
  removeTask(id: string): void { this.cancelTask(id); }

  cancelTask(id: string): void { this.setTaskStatus(id, 'cancelled'); }

  completeTask(id: string, result?: any): void { this.setTaskStatus(id, 'completed', result); }

  failTask(id: string, error?: any): void { this.setTaskStatus(id, 'failed', undefined, error); }
  ...
  resetTask(id: string): void {
    ...
    const ds = new Map<string, DepStatus>();
    for (const dep of task.deps) ds.set(dep, 'pending');
    task.depStatus = ds;
    this.evaluateAndMove(id);
  ...
  addDependency(taskId: string, depId: string): void {
    ...
    if (depId === taskId) throw new Error('Self-dependency not allowed');
    if (task.depsSet!.has(depId)) return;

    const newDeps = new Set(task.depsSet!).add(depId);
    if (this.detectCycle(taskId, newDeps)) throw new Error(`Cycle detected adding dep ${depId} to ${taskId}`);

    task.deps = Array.from(newDeps);
    task.depsSet = newDeps;
    this.bumpVersion(taskId);

    const rev = this.reverse.get(depId) ?? new Set<string>();
    rev.add(taskId);
  ...
  dequeueNextExecutable(now?: number): Task | null {
    ...
    while (this.readyHeap.size > 0) {
      ...
      if (!task) continue;
      if (task.status !== 'pending') continue;       // terminal / running
      if (entry.seq !== task.version) continue;       // stale (version bumped)
  ...
  getExecutableTasks(now?: number): Task[] {
    ...
    this.moveDeferredToReady(t0);
    const result: Task[] = [];
    const temp: HeapEntry[] = [];
    while (this.readyHeap.size > 0) {
      const entry = this.readyHeap.pop();
      if (!entry) break;
      const task = this.tasks.get(entry.task.id);
      const valid = task && task.status === 'pending' && entry.seq === task.version;
      if (valid) result.push(task);
      temp.push(entry);
    }
    for (const e of temp) this.readyHeap.push(e);
    return result;
  }
  ...
  getDeferredTasks(now?: number): Task[] {
    ...
    for (const e of temp) this.deferredHeap.push(e);
    return result;
  }
  ...
  getPendingTasks(): Task[] {
    ...
    for (const task of this.tasks.values()) {
      if (task.status === 'pending') result.push(task);
    }
    return result;
  }
  ...
  getBlockedTasks(): Task[] {
    const result: Task[] = [];
    for (const task of this.tasks.values()) {
      if (task.status === 'blocked') result.push(task);
    }
    return result;
  }
  ...
  // ------------------------------------------------------------- internals
  private generateId(): string { return `t${this.nextIdSeq++}`; }

  private bumpVersion(id: string): number {
    const task = this.tasks.get(id)!;
    task.version = (task.version ?? 0) + 1;
    return task.version;
  }

  private getStatusCounts(task: Task): { completed: number; failed: number; pending: number } {
   ...
   */
  private detectCycle(targetId: string, deps: Set<string>): boolean {
    const visited = new Set<string>();
    for (const dep of deps) {
      if (this.dfsReaches(dep, targetId, visited)) return true;
    }
    return false;
  }

  private dfsReaches(start: string, target: string, visited: Set<string>): boolean {
  ...
  private initDepStatus(task: Task, deps: Set<string>): void {
    const ds = new Map<string, DepStatus>();
    for (const dep of deps) {
      const dt = this.tasks.get(dep);
      if (!dt) { ds.set(dep, 'pending'); continue; } // forward reference
      if (dt.status === 'completed') ds.set(dep, 'completed');
      else if (dt.status === 'failed' || dt.status === 'cancelled') ds.set(dep, 'failed');
      else ds.set(dep, 'pending');
    }
    task.depStatus = ds;
  }

  /** Push a ready task onto the ready or deferred heap. */
  private moveToReadyOrDeferred(id: string): void {
    const task = this.tasks.get(id);
    if (!task || task.status !== 'pending') return;
    const entry: HeapEntry = { task, seq: task.version };
    if (task.scheduledAt <= this.now) this.readyHeap.push(entry);
    else this.deferredHeap.push(entry);
  }

  /** Re-evaluate a task and move it to a heap when it becomes ready. */
  private evaluateAndMove(id: string): void {
    const task = this.tasks.get(id);
    if (!task || task.status !== 'pending') return;
    if (!this.isReady(task)) return;
   ...
   */
  private moveDeferredToReady(now: number): void {
    while (this.deferredHeap.size > 0) {
      const entry = this.deferredHeap.peek();
      if (!entry) break;
      const task = this.tasks.get(entry.task.id);
      const valid = task && task.status === 'pending' && entry.seq === task.version;
      if (!valid) { this.deferredHeap.pop(); continue; }
      if (task.scheduledAt > now) break;
   ...
   */
  private setTaskStatus(id: string, status: TaskStatus, result?: any, error?: any): void {
    const task = this.tasks.get(id);
    if (!task) throw new Error(`Task ${id} not found`);
    const oldStatus = task.status;
   ...
   */
  private propagate(id: string, status: TaskStatus, error?: any): void {
    const dependents = this.reverse.get(id);
    if (!dependents) return;
    for (const depId of dependents) {
      const dep = this.tasks.get(depId);
      if (!dep || dep.status === 'blocked' || dep.status === 'completed' || dep.status === 'failed' || dep.status === 'cancelled') continue;
  ...
  private fireStatusChange(id: string, oldStatus: TaskStatus, newStatus: TaskStatus, payload?: any): void {
    for (const cb of this.listeners) {
      try { cb(id, oldStatus, newStatus, payload); } catch (err) { void err; }
    }
  }
}
