...
| 7 | Ornith-1.5-35B-A3B-oQ4e-mtp | 23 | 25/25 ✅ | 1.28s | Best architecture doc; `sinkDown` heap-invariant bug |
| 8 | KAT-Coder-V2.5-Dev-VL-oQ4e-mtp | 19.5 | 24/24 ✅ | 10k in 3.0s | 708 TPS serving; O(n) heap.clean() per retrieval |
| 9 | Muse-Glimmer-30B-oQ4e | 17 | 17/21 ❌ | — | Syntax error in source; 4 tests fail on own expectations |
| 10 | NeoHorse-1-9B-oQ6e | 13.5 | 10/23 ❌ | — | Heap pop/delete crash; stats accumulate without reset |
...
**1. Serving speed ≠ code quality.** KAT-Coder served at 708 TPS (fastest) and produced a heap that rebuilds itself O(n) on every retrieval. Agnes served at 74.6 TPS (second slowest) and produced the winning implementation. NeoHorse spent 9 hours generating a scheduler that crashes on its own test suite.
